...
from nutella.product.management.filter_file import extract_datas, delete_duplicated_products
...
class DownloadProducts(TestCase):
    def test__extract_datas(self):
        ...
        self.assertEqual(extract_datas(products_data, "Produits laitiers"), cleaned_products)

    def test_eliminate_duplicated_products(self):
        with open(
            "fixtures/data_double.json", encoding="utf-8-sig"
        ) as f:
            list_with_double = json.load(f)
        with open(
            "fixtures/double_deleted.json", encoding="utf-8-sig"
        ) as f:
